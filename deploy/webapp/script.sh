...
mysql -h $DBhostname -P 3306 -u$DBuser -p$DBpass -e 'use csye7374'
if [ $? -eq 0 ]
then
    echo "database $DBNAME exist."
	mysql -h $DBhostname -P 3306 -u$DBuser -p$DBpass -e 'Select * from csye7374.user'
	if [ $? -eq 0 ]
	then
		echo "Schema exist"
	else
	...
	mysql -h $DBhostname -P 3306 -u$DBuser -p$DBpass -e 'CREATE DATABASE csye7374'
	mysql -h $DBhostname -P 3306 -u$DBuser -p$DBpass < /tmp/tables.sql
